...
public class BackFeedController implements Initializable {
    ...
    private VBox createFeedbackCard(Feed feedback) {
        ...
        SentimentType sentiment = analysisService.analyzeFeedbackSentiment(feedback.getCommentaire_feed());
    ...
    private void filterFeedbacks(String searchText) {
        ...
        List<Feed> filtered = allFeedbacks.stream()
                .filter(feedback -> 
                    feedback.getName_feed().toLowerCase().contains(searchText.toLowerCase()) ||
                    feedback.getSubject_feed().toLowerCase().contains(searchText.toLowerCase()) ||
                    feedback.getCommentaire_feed().toLowerCase().contains(searchText.toLowerCase()))
                .collect(Collectors.toList());
